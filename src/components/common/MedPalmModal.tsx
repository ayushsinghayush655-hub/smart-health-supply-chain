...
export const MedPalmModal: React.FC<MedPalmModalProps> = ({
  ...
  const runMedPalmSynthesis = async () => {
    ...
    try {
      ...
      console.error('Gemini trigger error:', e);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/80 backdrop-blur-md p-4 overflow-y-auto">
      <div className="bg-slate-50 border border-slate-300 rounded-3xl w-full max-w-4xl shadow-2xl text-slate-900 max-h-[90vh] flex flex-col overflow-hidden">
        {/* Modal Header */}
        <div className="p-5 bg-gradient-to-r from-white via-indigo-50 to-white border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-700 border border-indigo-500/40 uppercase tracking-wider">
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="bg-white/60 p-4 rounded-2xl border border-slate-200">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
              <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-2">
                ...
                <div>
                  <div className="font-bold text-slate-800">CWC River Feed</div>
                  <div className="text-[10px] text-slate-600">114.8m Warning Level</div>
                </div>
              </div>
              ...
              <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-2">
                ...
                <div>
                  <div className="font-bold text-slate-800">IMD Meteorology</div>
                  <div className="text-[10px] text-slate-600">62mm/hr Heavy Rain</div>
                </div>
              </div>
              ...
              <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-2">
                ...
                <div>
                  <div className="font-bold text-slate-800">Epidemiology Dept</div>
                  <div className="text-[10px] text-slate-600">Gastro & Dengue Clusters</div>
                </div>
              </div>
              ...
              <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-2">
                ...
                <div>
                  <div className="font-bold text-slate-800">PHC & Hospital Beds</div>
                  <div className="text-[10px] text-slate-600">Real-time Dispensing & Load</div>
                </div>
              </div>
            </div>
          </div>
          ...
          {!analysis && !isLoading && (
            <div className="text-center py-10 space-y-4">
              <div className="w-16 h-16 rounded-2xl bg-indigo-500/10 text-indigo-600 border border-indigo-500/20 flex items-center justify-center mx-auto">
                <Sparkles className="w-8 h-8" />
              </div>
              <div className="max-w-md mx-auto space-y-1">
                <h3 className="text-base font-bold text-slate-900">
                  Execute Gemini Unified Epidemiological Analysis
                </h3>
              ...
              <button
                ...
                className="px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-slate-900 rounded-xl text-xs font-bold shadow-xl transition cursor-pointer"
              >
            ...
            <div className="py-16 text-center space-y-4">
              ...
              <div className="space-y-1">
                <p className="text-sm font-bold text-slate-900">
                  Correlating Multi-Agency Feeds with Gemini...
                </p>
                <p className="text-xs text-slate-600">
            ...
            <div className="space-y-6">
              ...
              <div className="p-4 rounded-2xl bg-white border border-slate-200 space-y-3">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-rose-500/20 text-rose-700 border border-rose-500/30">
                ...
                <div className="pt-2 border-t border-slate-200/80 flex items-center gap-2 flex-wrap">
                  <span className="text-xs font-semibold text-slate-600">Primary Outbreak Risks:</span>
                  {analysis.threatAssessment.primaryDiseases.map((dis, idx) => (
                    <span key={idx} className="text-xs px-2 py-0.5 rounded bg-slate-100 text-slate-800 border border-slate-300">
                      {dis}
                    </span>
                  ))}
                </div>
              ...
              <div className="space-y-3">
                <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-2">
                  ...
                  Gemini Identified Critical Medicine Requirements
                </h4>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {analysis.medicineRequirements.map((med, idx) => (
                    <div
                      key={idx}
                      className="bg-white p-3.5 rounded-xl border border-slate-200 space-y-1.5"
                    >
                  ...
                  {analysis.logisticsActions.map((action, idx) => (
                    ...
                    >
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-700">
                            {action.type}
                          </span>
                      ...
                      <span className="text-[10px] font-bold px-2 py-1 rounded bg-amber-500/10 text-amber-700 border border-amber-500/20 shrink-0">
        ...
        <div className="p-4 bg-slate-100/80 border-t border-slate-300 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onClose}
              className="px-4 py-2 rounded-xl text-xs text-slate-700 hover:bg-slate-700 transition"
            >
              Close
            </button>
            {analysis && (
