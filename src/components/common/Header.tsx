...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header id="aarogya-main-header" className="w-full bg-slate-900 text-slate-100 border-b border-slate-800 sticky top-0 z-40 shadow-md">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2.5 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-amber-500/10 border border-amber-400/30 flex items-center justify-center text-amber-300 shadow-inner">
          ...
          <div>
            ...
            <h1 className="text-base sm:text-lg font-bold tracking-tight text-white flex items-center gap-2">
              Vitality <span className="text-xs font-normal text-slate-400 hidden sm:inline">| {getDashboardLabel()}</span>
            </h1>
          </div>
        </div>

        {/* Center: Med-PaLM & Sync Controls */}
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          {/* Med-PaLM Engine Trigger */}
          <button
            id="btn-open-medpalm"
            onClick={onOpenMedPalm}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-950/80 hover:bg-indigo-900 text-indigo-200 border border-indigo-700/50 text-xs font-medium transition shadow-sm"
            title="Open Multi-Source Med-PaLM Threat & Supply Synthesizer"
          >
            <Sparkles className="w-3.5 h-3.5 text-indigo-400 animate-pulse" />
            <span className="hidden sm:inline">Med-PaLM</span> AI Synthesizer
          </button>

          {/* Network Connectivity Toggle (Real & Simulation) */}
          <div className="flex items-center rounded-lg border border-slate-700 bg-slate-800/80 px-2.5 py-1 text-xs">
            <button
              id="btn-toggle-connectivity"
              onClick={handleToggleOffline}
              className={`flex items-center gap-1.5 font-medium transition cursor-pointer ${
                isOnline ? 'text-emerald-400' : 'text-rose-400 font-semibold'
              }`}
            ...
            {pendingSyncCount > 0 && (
              <button
                ...
                className="ml-2 pl-2 border-l border-slate-700 flex items-center gap-1 text-[11px] text-amber-300 hover:text-amber-200 disabled:opacity-50"
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-xs text-slate-200"
            >
              ...
              <div className="text-left hidden md:block">
                ...
                <div className="text-[10px] text-slate-400">{userSession.facilityName}</div>
              </div>
              <ChevronDown className="w-3.5 h-3.5 text-slate-400" />
            </button>

            {showRoleMenu && (
              <div className="absolute right-0 mt-1 w-64 rounded-xl bg-slate-800 border border-slate-700 shadow-xl py-1.5 z-50 text-xs">
      ...
      {/* Sync Toast Alert */}
      {syncToast && (
        <div className="bg-emerald-900/90 text-emerald-100 text-xs px-4 py-1.5 text-center flex items-center justify-center gap-2 border-t border-emerald-700">
